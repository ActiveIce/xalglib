...
static void mlptrain_mlpkfoldcvgeneral(const multilayerperceptron* n,
     ...
     mlpcvreport* cvrep,
     ae_state *_state);
static void mlptrain_mlpkfoldsplit(/* Real    */ const ae_matrix* xy,
     ae_int_t npoints,
     ae_int_t nclasses,
     ae_int_t foldscount,
...
static void mlptrain_mthreadcv(mlptrainer* s,
     ...
     ae_int_t wcount,
     ae_state *_state);
void _spawn_mlptrain_mthreadcv(mlptrainer* s,
    ae_int_t rowsize,
    ae_int_t nrestarts,
    /* Integer */ const ae_vector* folds,
    ae_int_t fold,
    ...
    /* Real    */ ae_matrix* cvy,
    ae_shared_pool* pooldatacv,
    ae_int_t wcount, ae_task_group *_group, ae_bool smp_enabled, ae_state *_state);
#if defined(_ALGLIB_HAS_WORKSTEALING)
void _task_mlptrain_mthreadcv(ae_task_data *_data, ae_state *_state);
#endif
...
static void mlptrain_mlptrainnetworkx(const mlptrainer* s,
     ae_int_t nrestarts,
     ae_int_t algokind,
     /* Integer */ const ae_vector* trnsubset,
     ae_int_t trnsubsetsize,
     /* Integer */ const ae_vector* valsubset,
     ...
     ae_state *_state);
void _spawn_mlptrain_mlptrainnetworkx(const mlptrainer* s,
    ae_int_t nrestarts,
    ae_int_t algokind,
    /* Integer */ const ae_vector* trnsubset,
    ae_int_t trnsubsetsize,
    ...
    mlpreport* rep,
    ae_bool isrootcall,
    ae_shared_pool* sessions, ae_task_group *_group, ae_bool smp_enabled, ae_state *_state);
#if defined(_ALGLIB_HAS_WORKSTEALING)
void _task_mlptrain_mlptrainnetworkx(ae_task_data *_data, ae_state *_state);
#endif
ae_bool _trypexec_mlptrain_mlptrainnetworkx(const mlptrainer* s,
    ae_int_t nrestarts,
    ae_int_t algokind,
    /* Integer */ const ae_vector* trnsubset,
    ae_int_t trnsubsetsize,
...
static void mlptrain_mlptrainensemblex(const mlptrainer* s,
     ...
     ae_state *_state);
void _spawn_mlptrain_mlptrainensemblex(const mlptrainer* s,
    mlpensemble* ensemble,
    ae_int_t idx0,
    ae_int_t idx1,
    ae_int_t nrestarts,
    ...
    sinteger* ngrad,
    ae_bool isrootcall,
    ae_shared_pool* esessions, ae_task_group *_group, ae_bool smp_enabled, ae_state *_state);
#if defined(_ALGLIB_HAS_WORKSTEALING)
void _task_mlptrain_mlptrainensemblex(ae_task_data *_data, ae_state *_state);
#endif
ae_bool _trypexec_mlptrain_mlptrainensemblex(const mlptrainer* s,
    mlpensemble* ensemble,
    ae_int_t idx0,
    ae_int_t idx1,
    ae_int_t nrestarts,
...
static void mlptrain_mlpstarttrainingx(const mlptrainer* s,
     ae_bool randomstart,
     ae_int_t algokind,
     ...
     ae_state *_state);
static ae_bool mlptrain_mlpcontinuetrainingx(const mlptrainer* s,
     /* Integer */ const ae_vector* subset,
     ae_int_t subsetsize,
     ae_int_t* ngradbatch,
     smlptrnsession* session,
...
static void mlptrain_mlpebagginginternal(mlpensemble* ensemble,
     ...
     ae_state *_state);
static void mlptrain_initmlptrnsession(const multilayerperceptron* networktrained,
     ae_bool randomizenetwork,
     const mlptrainer* trainer,
     smlptrnsession* session,
     ae_state *_state);
static void mlptrain_initmlptrnsessions(const multilayerperceptron* networktrained,
     ae_bool randomizenetwork,
     const mlptrainer* trainer,
     ae_shared_pool* sessions,
     ae_state *_state);
static void mlptrain_initmlpetrnsession(const multilayerperceptron* individualnetwork,
     const mlptrainer* trainer,
     mlpetrnsession* session,
     ae_state *_state);
static void mlptrain_initmlpetrnsessions(const multilayerperceptron* individualnetwork,
     const mlptrainer* trainer,
     ae_shared_pool* sessions,
     ae_state *_state);
    ...
    *info = 2;
    ...
    for(pass=1; pass<=restarts; pass++)
    {
        ...
        for(;;)
        {
            ...
            spdmatrixcholeskysolve(&hmod, wcount, ae_true, &g, &wdir, &solverrep, _state);
            ...
            ae_v_muld(&wdir.ptr.p_double[0], 1, ae_v_len(0,wcount-1), -1.0);
            ...
            if( ae_fp_less(stepnorm,lmsteptol*((double)1+ae_sqrt(xnorm2, _state))) )
            {
                break;
            }
             ...
             */
            rmatrixtrinverse(&hmod, wcount, ae_true, ae_false, &invrep, _state);
    ...
    for(pass=1; pass<=restarts; pass++)
    {
        ...
        while(minlbfgsiteration(&state, _state))
        {
            ...
            if( state.xupdated )
            {
                ...
                if( itcnt>30&&ae_fp_greater((double)(itcnt),1.5*(double)itbest) )
                {
                    *info = 6;
                    break;
                }
     ...
     ae_state *_state)
{
     ...
     * Create shared pool
     */
    ae_shared_pool_set_seed(&pooldatacv, &datacv, (ae_int_t)sizeof(datacv), (ae_constructor)_mlpparallelizationcv_init, (ae_copy_constructor)_mlpparallelizationcv_init_copy, (ae_destructor)_mlpparallelizationcv_destroy, _state);
    ...
    *info = 0;
    ...
    for(fold=0; fold<=foldscount-1; fold++)
    {
        ...
        if( mlpissoftmax(&network, _state) )
        {
             ...
             */
            cvrep->relclserror = cvrep->relclserror+(double)mlpclserror(&network, &testset, tssize, _state);
        ...
        for(i=0; i<=tssize-1; i++)
        {
            ...
            if( mlpissoftmax(&network, _state) )
            {
                ...
                for(j=0; j<=nout-1; j++)
                {
                    if( j==k )
                    {
                        cvrep->rmserror = cvrep->rmserror+ae_sqr(y.ptr.p_double[j]-(double)1, _state);
                        cvrep->avgerror = cvrep->avgerror+ae_fabs(y.ptr.p_double[j]-(double)1, _state);
                        cvrep->avgrelerror = cvrep->avgrelerror+ae_fabs(y.ptr.p_double[j]-(double)1, _state);
                        relcnt = relcnt+1;
                    }
                    else
                    {
    ...
    if( mlpissoftmax(&network, _state) )
    {
        cvrep->relclserror = cvrep->relclserror/(double)npoints;
        cvrep->avgce = cvrep->avgce/(ae_log((double)(2), _state)*(double)npoints);
    }
    cvrep->rmserror = ae_sqrt(cvrep->rmserror/(double)(npoints*nout), _state);
    cvrep->avgerror = cvrep->avgerror/(double)(npoints*nout);
    if( relcnt>0 )
    {
        cvrep->avgrelerror = cvrep->avgrelerror/(double)relcnt;
    }
    *info = 1;
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    else
    {
         ...
         */
        if( ae_fp_greater_eq((double)ae_maxint(nrestarts, 1, _state)*rmul3((double)(2*wcount), (double)(s->npoints), (double)(100), _state),smpactivationlevel(_state)) )
        {
         ...
         * Split task
         */
        _spawn_mlptrain_mthreadcv(s, rowsize, nrestarts, folds, fold, (fold+dfold)/2, cvy, pooldatacv, wcount, _child_tasks, _smp_enabled, _state);
        _spawn_mlptrain_mthreadcv(s, rowsize, nrestarts, folds, (fold+dfold)/2, dfold, cvy, pooldatacv, wcount, _child_tasks, _smp_enabled, _state);
    ...
    ae_task_group *_group, ae_bool smp_enabled, ae_state *_state)
{
    ...
    {
        mlptrain_mthreadcv(s,rowsize,nrestarts,folds,fold,dfold,cvy,pooldatacv,wcount, _state);
        return;
    }
#if defined(_ALGLIB_HAS_WORKSTEALING)
    {
        ...
        _task->data.func = _task_mlptrain_mthreadcv;
        _task->data.flags = _state->flags;
        _task->data.parameters[0].value.val = s;
        _task->data.parameters[1].value.ival = rowsize;
        _task->data.parameters[2].value.ival = nrestarts;
        _task->data.parameters[3].value.const_val = folds;
        _task->data.parameters[4].value.ival = fold;
        _task->data.parameters[5].value.ival = dfold;
        _task->data.parameters[6].value.val = cvy;
        _task->data.parameters[7].value.val = pooldatacv;
        _task->data.parameters[8].value.ival = wcount;
...
void _task_mlptrain_mthreadcv(ae_task_data *_data, ae_state *_state)
{
    ...
    ae_int_t rowsize;
    ae_int_t nrestarts;
    const ae_vector* folds;
    ae_int_t fold;
    ae_int_t dfold;
    ae_matrix* cvy;
    ae_shared_pool* pooldatacv;
    ae_int_t wcount;
    s = (mlptrainer*)_data->parameters[0].value.val;
    rowsize = _data->parameters[1].value.ival;
    nrestarts = _data->parameters[2].value.ival;
    folds = (const ae_vector*)_data->parameters[3].value.const_val;
    fold = _data->parameters[4].value.ival;
    dfold = _data->parameters[5].value.ival;
    cvy = (ae_matrix*)_data->parameters[6].value.val;
    pooldatacv = (ae_shared_pool*)_data->parameters[7].value.val;
    wcount = _data->parameters[8].value.ival;
     ...
     ae_state *_state)
{
    ...
    if( nrestarts>=2 )
    {
        ...
        nr0 = nrestarts/2;
        nr1 = nrestarts-nr0;
        _spawn_mlptrain_mlptrainnetworkx(s, nr0, algokind, trnsubset, trnsubsetsize, valsubset, valsubsetsize, network, &rep0, ae_false, sessions, _child_tasks, _smp_enabled, _state);
    ...
    ae_task_group *_group, ae_bool smp_enabled, ae_state *_state)
{
    if( _group==NULL || !smp_enabled)
    {
        mlptrain_mlptrainnetworkx(s,nrestarts,algokind,trnsubset,trnsubsetsize,valsubset,valsubsetsize,network,rep,isrootcall,sessions, _state);
        return;
    }
#if defined(_ALGLIB_HAS_WORKSTEALING)
    {
        ae_task_info *_task;
        _task = ae_create_task(_group, _state);
        _task->data.func = _task_mlptrain_mlptrainnetworkx;
        _task->data.flags = _state->flags;
        _task->data.parameters[0].value.const_val = s;
        _task->data.parameters[1].value.ival = nrestarts;
        _task->data.parameters[2].value.ival = algokind;
        _task->data.parameters[3].value.const_val = trnsubset;
        _task->data.parameters[4].value.ival = trnsubsetsize;
        _task->data.parameters[5].value.const_val = valsubset;
        _task->data.parameters[6].value.ival = valsubsetsize;
        _task->data.parameters[7].value.val = network;
        _task->data.parameters[8].value.val = rep;
        _task->data.parameters[9].value.bval = isrootcall;
        _task->data.parameters[10].value.val = sessions;
        ae_push_task(_task, _state, ae_false);
    }
#else
    abort(); /* critical integrity failure */
...
void _task_mlptrain_mlptrainnetworkx(ae_task_data *_data, ae_state *_state)
{
    ...
    ae_int_t nrestarts;
    ae_int_t algokind;
    const ae_vector* trnsubset;
    ae_int_t trnsubsetsize;
    const ae_vector* valsubset;
    ae_int_t valsubsetsize;
    multilayerperceptron* network;
    mlpreport* rep;
    ae_bool isrootcall;
    ae_shared_pool* sessions;
    s = (const mlptrainer*)_data->parameters[0].value.const_val;
    nrestarts = _data->parameters[1].value.ival;
    algokind = _data->parameters[2].value.ival;
    trnsubset = (const ae_vector*)_data->parameters[3].value.const_val;
    trnsubsetsize = _data->parameters[4].value.ival;
    valsubset = (const ae_vector*)_data->parameters[5].value.const_val;
    valsubsetsize = _data->parameters[6].value.ival;
    network = (multilayerperceptron*)_data->parameters[7].value.val;
    rep = (mlpreport*)_data->parameters[8].value.val;
    isrootcall = _data->parameters[9].value.bval;
    sessions = (ae_shared_pool*)_data->parameters[10].value.val;
   ae_state_set_flags(_state, _data->flags);
   mlptrain_mlptrainnetworkx(s,nrestarts,algokind,trnsubset,trnsubsetsize,valsubset,valsubsetsize,network,rep,isrootcall,sessions, _state);
}
#endif
     ...
     ae_state *_state)
{
    ...
    if( isrootcall )
    {
         ...
         * Hence is our approach to evaluation of task complexity.
         */
        if( ae_fp_greater_eq((double)(ae_maxint(nrestarts, 1, _state)*(idx1-idx0))*rmul3((double)(2*wcount), (double)(s->npoints), (double)(100), _state),smpactivationlevel(_state)) )
        {
    ...
    if( idx1-idx0>=2 )
    {
        ...
        ngrad0.val = 0;
        ngrad1.val = 0;
        _spawn_mlptrain_mlptrainensemblex(s, ensemble, idx0, idx0+k0, nrestarts, trainingmethod, &ngrad0, ae_false, esessions, _child_tasks, _smp_enabled, _state);
        _spawn_mlptrain_mlptrainensemblex(s, ensemble, idx0+k0, idx1, nrestarts, trainingmethod, &ngrad1, ae_false, esessions, _child_tasks, _smp_enabled, _state);
        ae_sync(_child_tasks, _smp_enabled, ae_false, _state);
    ...
    ae_task_group *_group, ae_bool smp_enabled, ae_state *_state)
{
    if( _group==NULL || !smp_enabled)
    {
        mlptrain_mlptrainensemblex(s,ensemble,idx0,idx1,nrestarts,trainingmethod,ngrad,isrootcall,esessions, _state);
        return;
    }
#if defined(_ALGLIB_HAS_WORKSTEALING)
    {
        ae_task_info *_task;
        _task = ae_create_task(_group, _state);
        _task->data.func = _task_mlptrain_mlptrainensemblex;
        _task->data.flags = _state->flags;
        _task->data.parameters[0].value.const_val = s;
        _task->data.parameters[1].value.val = ensemble;
        _task->data.parameters[2].value.ival = idx0;
        _task->data.parameters[3].value.ival = idx1;
        _task->data.parameters[4].value.ival = nrestarts;
        _task->data.parameters[5].value.ival = trainingmethod;
        _task->data.parameters[6].value.val = ngrad;
        _task->data.parameters[7].value.bval = isrootcall;
        _task->data.parameters[8].value.val = esessions;
        ae_push_task(_task, _state, ae_false);
    }
#else
    abort(); /* critical integrity failure */
...
void _task_mlptrain_mlptrainensemblex(ae_task_data *_data, ae_state *_state)
{
    ...
    ae_shared_pool* esessions;
    s = (const mlptrainer*)_data->parameters[0].value.const_val;
    ensemble = (mlpensemble*)_data->parameters[1].value.val;
    idx0 = _data->parameters[2].value.ival;
    idx1 = _data->parameters[3].value.ival;
    nrestarts = _data->parameters[4].value.ival;
    trainingmethod = _data->parameters[5].value.ival;
    ngrad = (sinteger*)_data->parameters[6].value.val;
    isrootcall = _data->parameters[7].value.bval;
    esessions = (ae_shared_pool*)_data->parameters[8].value.val;
   ae_state_set_flags(_state, _data->flags);
   mlptrain_mlptrainensemblex(s,ensemble,idx0,idx1,nrestarts,trainingmethod,ngrad,isrootcall,esessions, _state);
}
#endif
     ...
     ae_state *_state)
{
    ...
    else
    {
        ...
        ae_shared_pool_set_seed(sessions, &t, (ae_int_t)sizeof(t), (ae_constructor)_smlptrnsession_init, (ae_copy_constructor)_smlptrnsession_init_copy, (ae_destructor)_smlptrnsession_destroy, _state);
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    if( !ae_shared_pool_is_initialized(sessions) )
    {
        ...
        ae_shared_pool_set_seed(sessions, &t, (ae_int_t)sizeof(t), (ae_constructor)_mlpetrnsession_init, (ae_copy_constructor)_mlpetrnsession_init_copy, (ae_destructor)_mlpetrnsession_destroy, _state);
    }
    ae_frame_leave(_state);
}
