...
static void mlpbase_mlpcreate(ae_int_t nin,
     ae_int_t nout,
     /* Integer */ const ae_vector* lsizes,
     /* Integer */ const ae_vector* ltypes,
     /* Integer */ const ae_vector* lconnfirst,
     /* Integer */ const ae_vector* lconnlast,
     ae_int_t layerscount,
     ae_bool isclsnet,
     multilayerperceptron* network,
     ae_state *_state);
static void mlpbase_mlpgetneuroninfox(const multilayerperceptron* network,
     ae_int_t k,
     ae_int_t i,
     /* Integer */ ae_vector* integerbuf,
     ae_int_t* fkind,
     double* threshold,
     ae_state *_state);
static double mlpbase_mlpgetweightx(const multilayerperceptron* network,
     ae_int_t k0,
     ae_int_t i0,
     ae_int_t k1,
     ae_int_t i1,
...
static void mlpbase_mlpinternalcalculategradient(multilayerperceptron* network,
     /* Real    */ const ae_vector* neurons,
     /* Real    */ const ae_vector* weights,
     /* Real    */ ae_vector* derror,
     /* Real    */ ae_vector* grad,
     ae_bool naturalerrorfunc,
     ae_state *_state);
static void mlpbase_mlpchunkedgradient(const multilayerperceptron* network,
     /* Real    */ const ae_matrix* xy,
     ae_int_t cstart,
     ae_int_t csize,
     ...
     ae_bool naturalerrorfunc,
     ae_state *_state);
static void mlpbase_mlpchunkedprocess(const multilayerperceptron* network,
     /* Real    */ ae_matrix* xy,
     ae_int_t cstart,
     ae_int_t csize,
     /* Real    */ ae_vector* batch4buf,
     ...
     ae_state *_state)
{
    ...
    ae_shared_pool_set_seed(&network2->buf, &buf, (ae_int_t)sizeof(buf), (ae_constructor)_mlpbuffers_init, (ae_copy_constructor)_mlpbuffers_init_copy, (ae_destructor)_mlpbuffers_destroy, _state);
    ...
    ae_shared_pool_set_seed(&network2->gradbuf, &sgrad, (ae_int_t)sizeof(sgrad), (ae_constructor)_smlpgrad_init, (ae_copy_constructor)_smlpgrad_init_copy, (ae_destructor)_smlpgrad_destroy, _state);
    ae_frame_leave(_state);
}
...
void mlprandomize(multilayerperceptron* network, ae_state *_state)
{
    ...
    for(neuronidx=0; neuronidx<=ntotal-1; neuronidx++)
    {
        ...
        if( neurontype>0 )
        {
            ...
            ef = ef/(double)montecarlocnt;
            ef2 = ef2/(double)montecarlocnt;
    ...
    if( !mlpissoftmax(network, _state) )
    {
        for(i=0; i<=nout-1; i++)
        {
            ...
            if( ntype==0 )
            {
                 ...
                 */
                network->columnmeans.ptr.p_double[nin+i] = (double)2*ae_randomreal(_state)-(double)1;
            }
            if( ntype==0||ntype==3 )
            {
                 ...
                 */
                network->columnsigmas.ptr.p_double[nin+i] = (double)ae_sign(network->columnsigmas.ptr.p_double[nin+i], _state)*(1.5*ae_randomreal(_state)+0.5);
            }
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=jmax; i++)
    {
        means.ptr.p_double[i] = means.ptr.p_double[i]/(double)ssize;
    }
    for(i=0; i<=ssize-1; i++)
    {
        for(j=0; j<=jmax; j++)
    ...
    for(i=0; i<=jmax; i++)
    {
        sigmas.ptr.p_double[i] = ae_sqrt(sigmas.ptr.p_double[i]/(double)ssize, _state);
    }
    ...
    if( !mlpissoftmax(network, _state) )
    {
        for(i=0; i<=nout-1; i++)
        {
            ...
            if( ntype==3 )
            {
                ...
                network->columnsigmas.ptr.p_double[nin+i] = (double)ae_sign(network->columnsigmas.ptr.p_double[nin+i], _state)*ae_fabs(s, _state);
                if( ae_fp_eq(network->columnsigmas.ptr.p_double[nin+i],(double)(0)) )
                {
                    network->columnsigmas.ptr.p_double[nin+i] = (double)(1);
                }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=jmax; i++)
    {
        means.ptr.p_double[i] = means.ptr.p_double[i]/(double)subsetsize;
    }
    for(i=0; i<=subsetsize-1; i++)
    {
        for(j=0; j<=jmax; j++)
    ...
    for(i=0; i<=jmax; i++)
    {
        sigmas.ptr.p_double[i] = ae_sqrt(sigmas.ptr.p_double[i]/(double)subsetsize, _state);
    }
     ...
     ae_state *_state)
{
    ...
    *threshold = 0.0;
    ...
    mlpbase_mlpgetneuroninfox(network, k, i, &network->integerbuf, fkind, threshold, _state);
}
     ...
     ae_state *_state)
{
    ...
    result = mlpbase_mlpgetweightx(network, k0, i0, k1, i1, &network->integerbuf, _state);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    result = ae_sqr(network->err.rmserror, _state)*(double)npoints*(double)mlpgetoutputscount(network, _state)/(double)2;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    mlpallerrorsx(network, xy, &network->dummysxy, npoints, 0, &network->dummyidx, 0, npoints, 0, &network->buf, &network->err, _state);
    result = ae_round((double)npoints*network->err.relclserror, _state);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    *e = (double)(0);
    ...
    for(i=0; i<=nout-1; i++)
    {
        ...
        *e = *e+ae_sqr(network->y.ptr.p_double[i]-desiredy->ptr.p_double[i], _state)/(double)2;
    }
     ...
     ae_state *_state)
{
    ...
    problemcost = problemcost*(double)wcount*(double)2;
    ...
    if( subset1-subset0>=2*mlpbase_microbatchsize&&ae_fp_greater(problemcost,spawnlevel(_state)) )
    {
        ...
        _spawn_mlpgradbatchx(network, densexy, sparsexy, datasetsize, datasettype, idx, subset0, subset0+len0, subsettype, buf, gradbuf, _child_tasks, _smp_enabled, _state);
...
*************************************************************************/
void _spawn_mlpgradbatchx(const multilayerperceptron* network,
    /* Real    */ const ae_matrix* densexy,
    const sparsematrix* sparsexy,
    ae_int_t datasetsize,
    ae_int_t datasettype,
    ...
    ae_task_group *_group, ae_bool smp_enabled, ae_state *_state)
{
    ...
    {
        mlpgradbatchx(network,densexy,sparsexy,datasetsize,datasettype,idx,subset0,subset1,subsettype,buf,gradbuf, _state);
        return;
    }
#if defined(_ALGLIB_HAS_WORKSTEALING)
    {
        ...
        _task->data.func = _task_mlpgradbatchx;
        _task->data.flags = _state->flags;
        _task->data.parameters[0].value.const_val = network;
        _task->data.parameters[1].value.const_val = densexy;
        _task->data.parameters[2].value.const_val = sparsexy;
        _task->data.parameters[3].value.ival = datasetsize;
        _task->data.parameters[4].value.ival = datasettype;
        _task->data.parameters[5].value.const_val = idx;
        _task->data.parameters[6].value.ival = subset0;
        _task->data.parameters[7].value.ival = subset1;
        _task->data.parameters[8].value.ival = subsettype;
        _task->data.parameters[9].value.val = buf;
        _task->data.parameters[10].value.val = gradbuf;
...
void _task_mlpgradbatchx(ae_task_data *_data, ae_state *_state)
{
    ...
    ae_shared_pool* buf;
    ae_shared_pool* gradbuf;
    network = (const multilayerperceptron*)_data->parameters[0].value.const_val;
    densexy = (const ae_matrix*)_data->parameters[1].value.const_val;
    sparsexy = (const sparsematrix*)_data->parameters[2].value.const_val;
    datasetsize = _data->parameters[3].value.ival;
    datasettype = _data->parameters[4].value.ival;
    idx = (const ae_vector*)_data->parameters[5].value.const_val;
    subset0 = _data->parameters[6].value.ival;
    subset1 = _data->parameters[7].value.ival;
    subsettype = _data->parameters[8].value.ival;
    buf = (ae_shared_pool*)_data->parameters[9].value.val;
    gradbuf = (ae_shared_pool*)_data->parameters[10].value.val;
     ...
     ae_state *_state)
{
    ...
    ae_frame_make(_state, &_frame_block);
    memset(&integerbuf, 0, sizeof(integerbuf));
    ae_vector_init(&integerbuf, 0, DT_INT, _state, ae_true);
    ...
    for(i=1; i<=network->hllayersizes.cnt-1; i++)
    {
        ...
        {
            mlpbase_mlpgetneuroninfox(network, i, j, &integerbuf, &fkind, &threshold, _state);
            ae_serializer_alloc_entry(s);
            ae_serializer_alloc_entry(s);
     ...
     ae_state *_state)
{
    ...
    for(i=1; i<=network->hllayersizes.cnt-1; i++)
    {
        for(j=0; j<=network->hllayersizes.ptr.p_int[i]-1; j++)
        {
            ...
            for(k=0; k<=network->hllayersizes.ptr.p_int[i-1]-1; k++)
            {
                ae_serializer_serialize_double(s, mlpbase_mlpgetweightx(network, i-1, k, i, j, &integerbuf, _state), _state);
            }
        }
    }
    for(j=0; j<=nin-1; j++)
     ...
     ae_state *_state)
{
    ...
    result = ae_sqr(network->err.rmserror, _state)*(double)(idx1-idx0)*(double)mlpgetoutputscount(network, _state)/(double)2;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    if( subset1-subset0>=2*mlpbase_microbatchsize&&ae_fp_greater(problemcost,spawnlevel(_state)) )
    {
        ae_set_smp_support(&_child_tasks, &_smp_enabled, ae_true, _state);
        splitlength(subset1-subset0, mlpbase_microbatchsize, &len0, &len1, _state);
        _spawn_mlpallerrorsx(network, densexy, sparsexy, datasetsize, datasettype, idx, subset0, subset0+len0, subsettype, buf, &rep0, _child_tasks, _smp_enabled, _state);
        mlpallerrorsx(network, densexy, sparsexy, datasetsize, datasettype, idx, subset0+len0, subset1, subsettype, buf, &rep1, _state);
        ae_sync(_child_tasks, _smp_enabled, ae_false, _state);
        rep->relclserror = ((double)len0*rep0.relclserror+(double)len1*rep1.relclserror)/(double)(len0+len1);
        rep->avgce = ((double)len0*rep0.avgce+(double)len1*rep1.avgce)/(double)(len0+len1);
        rep->rmserror = ae_sqrt(((double)len0*ae_sqr(rep0.rmserror, _state)+(double)len1*ae_sqr(rep1.rmserror, _state))/(double)(len0+len1), _state);
        rep->avgerror = ((double)len0*rep0.avgerror+(double)len1*rep1.avgerror)/(double)(len0+len1);
        rep->avgrelerror = ((double)len0*rep0.avgrelerror+(double)len1*rep1.avgrelerror)/(double)(len0+len1);
        ae_sync(_child_tasks, _smp_enabled, ae_true, _state);
        ae_frame_leave(_state);
        return;
    }
...
Executed serially when called with NULL _group.
*************************************************************************/
void _spawn_mlpallerrorsx(const multilayerperceptron* network,
    /* Real    */ const ae_matrix* densexy,
    const sparsematrix* sparsexy,
    ae_int_t datasetsize,
    ae_int_t datasettype,
    ...
    ae_task_group *_group, ae_bool smp_enabled, ae_state *_state)
{
    if( _group==NULL || !smp_enabled)
    {
        mlpallerrorsx(network,densexy,sparsexy,datasetsize,datasettype,idx,subset0,subset1,subsettype,buf,rep, _state);
        return;
    }
#if defined(_ALGLIB_HAS_WORKSTEALING)
    {
        ae_task_info *_task;
        _task = ae_create_task(_group, _state);
        _task->data.func = _task_mlpallerrorsx;
        _task->data.flags = _state->flags;
        _task->data.parameters[0].value.const_val = network;
        _task->data.parameters[1].value.const_val = densexy;
        _task->data.parameters[2].value.const_val = sparsexy;
        ...
        _task->data.parameters[8].value.ival = subsettype;
        _task->data.parameters[9].value.val = buf;
        _task->data.parameters[10].value.val = rep;
        ae_push_task(_task, _state, ae_false);
    }
#else
    abort(); /* critical integrity failure */
...
void _task_mlpallerrorsx(ae_task_data *_data, ae_state *_state)
{
    ...
    subsettype = _data->parameters[8].value.ival;
    buf = (ae_shared_pool*)_data->parameters[9].value.val;
    rep = (modelerrors*)_data->parameters[10].value.val;
   ae_state_set_flags(_state, _data->flags);
   mlpallerrorsx(network,densexy,sparsexy,datasetsize,datasettype,idx,subset0,subset1,subsettype,buf,rep, _state);
}
#endif
     ...
     ae_state *_state)
{
     ...
     * Seed buffers
     */
    ae_shared_pool_set_seed(&network->buf, &buf, (ae_int_t)sizeof(buf), (ae_constructor)_mlpbuffers_init, (ae_copy_constructor)_mlpbuffers_init_copy, (ae_destructor)_mlpbuffers_destroy, _state);
    ae_vector_set_length(&sgrad.g, wcount, _state);
    sgrad.f = 0.0;
    for(i=0; i<=wcount-1; i++)
    {
    ...
    ae_shared_pool_set_seed(&network->gradbuf, &sgrad, (ae_int_t)sizeof(sgrad), (ae_constructor)_smlpgrad_init, (ae_copy_constructor)_smlpgrad_init_copy, (ae_destructor)_smlpgrad_destroy, _state);
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    *threshold = 0.0;
     ...
     * search
     */
    integerbuf->ptr.p_int[0] = k;
    integerbuf->ptr.p_int[1] = i;
    highlevelidx = recsearch(&network->hlneurons, mlpbase_hlnfieldwidth, 2, 0, ncnt, integerbuf, _state);
     ...
     ae_state *_state)
{
     ...
     * search
     */
    integerbuf->ptr.p_int[0] = k0;
    integerbuf->ptr.p_int[1] = i0;
    integerbuf->ptr.p_int[2] = k1;
    integerbuf->ptr.p_int[3] = i1;
    highlevelidx = recsearch(&network->hlconnections, mlpbase_hlconnfieldwidth, 4, 0, ccnt, integerbuf, _state);
    if( highlevelidx>=0 )
    {
            ...
            else
            {
                ...
                for(i=0; i<=nout-1; i++)
                {
                    ...
                    for(j=0; j<=nout-1; j++)
                    {
                        ...
                        if( j==i )
                        {
                            deidyj = expi/ae_sqr(s, _state)*((z+expi)*(s-(double)2*expi)/s+expi*s2/ae_sqr(s, _state));
                        }
                        else
                        {
                            deidyj = expi*expj/ae_sqr(s, _state)*(s2/ae_sqr(s, _state)-(double)2*z/s-(expi+expj)/s+(network->y.ptr.p_double[i]-desiredy.ptr.p_double[i])-(network->y.ptr.p_double[j]-desiredy.ptr.p_double[j]));
                        }
                        ae_v_addd(&rdy.ptr.pp_double[ntotal-nout+i][0], 1, &ry.ptr.pp_double[ntotal-nout+j][0], 1, ae_v_len(0,wcount-1), deidyj);
                    }
                }
