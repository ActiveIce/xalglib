...
static void clustering_selectinitialcenters(/* Real    */ const ae_matrix* xy,
     ae_int_t npoints,
     ae_int_t nvars,
     ae_int_t initalgo,
     hqrndstate* rs,
     ...
     ae_state *_state);
static ae_bool clustering_fixcenters(/* Real    */ const ae_matrix* xy,
     ae_int_t npoints,
     ae_int_t nvars,
     /* Real    */ ae_matrix* ct,
     ae_int_t k,
...
static void clustering_clusterizerrunahcinternal(clusterizerstate* s,
     ...
     ahcreport* rep,
     ae_state *_state);
static void clustering_evaluatedistancematrixrec(/* Real    */ const ae_matrix* xy,
     ae_int_t nfeatures,
     ae_int_t disttype,
     /* Real    */ ae_matrix* d,
     ae_int_t i0,
     ...
     ae_int_t j1,
     ae_state *_state);
void _spawn_clustering_evaluatedistancematrixrec(/* Real    */ const ae_matrix* xy,
    ae_int_t nfeatures,
    ae_int_t disttype,
    /* Real    */ ae_matrix* d,
    ae_int_t i0,
    ae_int_t i1,
    ae_int_t j0,
    ae_int_t j1, ae_task_group *_group, ae_bool smp_enabled, ae_state *_state);
#if defined(_ALGLIB_HAS_WORKSTEALING)
void _task_clustering_evaluatedistancematrixrec(ae_task_data *_data, ae_state *_state);
#endif
ae_bool _trypexec_clustering_evaluatedistancematrixrec(/* Real    */ const ae_matrix* xy,
    ae_int_t nfeatures,
    ae_int_t disttype,
    /* Real    */ ae_matrix* d,
    ae_int_t i0,
     ...
     ae_state *_state)
{
    ...
    if( disttype==2 )
    {
        ...
        for(i=0; i<=npoints-1; i++)
        {
            ...
            {
                v = ae_sqrt(ae_maxreal(buf->ra0.ptr.p_double[i]+buf->ra0.ptr.p_double[j]-(double)2*d->ptr.pp_double[i][j], 0.0, _state), _state);
    ...
    if( disttype==12||disttype==13 )
    {
        ...
        for(i=0; i<=npoints-1; i++)
        {
            ...
            for(j=i+1; j<=npoints-1; j++)
            {
                ...
                d->ptr.pp_double[i][j] = (double)1-v;
            }
        }
        rmatrixenforcesymmetricity(d, npoints, ae_true, _state);
        return;
    ...
    if( disttype==20||disttype==21 )
    {
        ...
        for(i=0; i<=npoints-1; i++)
        {
            ...
            {
                buf->ra0.ptr.p_double[i] = (double)1/ae_sqrt(d->ptr.pp_double[i][i], _state);
            }
            else
            {
     ...
     ae_state *_state)
{
    ...
    *k = 1;
    while(*k<rep->npoints&&ae_fp_greater_eq(rep->mergedist.ptr.p_double[rep->npoints-1-(*k)],(double)1-r))
    {
        *k = *k+1;
    }
    clusterizergetkclusters(rep, *k, cidx, cz, _state);
...
void kmeansinitbuf(kmeansbuffers* buf, ae_state *_state)
{
    ...
    ae_shared_pool_set_seed(&buf->updatepool, &updateseed, (ae_int_t)sizeof(updateseed), (ae_constructor)_apbuffers_init, (ae_copy_constructor)_apbuffers_init_copy, (ae_destructor)_apbuffers_destroy, _state);
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
     ...
     */
    rcomplexity = (double)2*rmul3((double)(idx1-idx0), (double)(cidx1-cidx0), (double)(nvars), _state);
    ...
    if( ((ae_fp_greater_eq(rcomplexity,spawnlevel(_state))&&idx1-idx0>=2*clustering_kmeansblocksize)&&nvars>=clustering_kmeansparalleldim)&&cidx1-cidx0>=clustering_kmeansparallelk )
    {
        ...
        _spawn_kmeansupdatedistances(xy, idx0, idx0+task0, nvars, ct, cidx0, cidx1, xyc, xydist2, bufferpool, _child_tasks, _smp_enabled, _state);
...
*************************************************************************/
void _spawn_kmeansupdatedistances(/* Real    */ const ae_matrix* xy,
    ae_int_t idx0,
    ae_int_t idx1,
    ae_int_t nvars,
    /* Real    */ const ae_matrix* ct,
    ...
    ae_task_group *_group, ae_bool smp_enabled, ae_state *_state)
{
    ...
    {
        kmeansupdatedistances(xy,idx0,idx1,nvars,ct,cidx0,cidx1,xyc,xydist2,bufferpool, _state);
        return;
    }
#if defined(_ALGLIB_HAS_WORKSTEALING)
    {
        ...
        _task->data.func = _task_kmeansupdatedistances;
        _task->data.flags = _state->flags;
        _task->data.parameters[0].value.const_val = xy;
        _task->data.parameters[1].value.ival = idx0;
        _task->data.parameters[2].value.ival = idx1;
        _task->data.parameters[3].value.ival = nvars;
        _task->data.parameters[4].value.const_val = ct;
        _task->data.parameters[5].value.ival = cidx0;
        _task->data.parameters[6].value.ival = cidx1;
        _task->data.parameters[7].value.val = xyc;
        _task->data.parameters[8].value.val = xydist2;
        _task->data.parameters[9].value.val = bufferpool;
...
void _task_kmeansupdatedistances(ae_task_data *_data, ae_state *_state)
{
    ...
    ae_vector* xydist2;
    ae_shared_pool* bufferpool;
    xy = (const ae_matrix*)_data->parameters[0].value.const_val;
    idx0 = _data->parameters[1].value.ival;
    idx1 = _data->parameters[2].value.ival;
    nvars = _data->parameters[3].value.ival;
    ct = (const ae_matrix*)_data->parameters[4].value.const_val;
    cidx0 = _data->parameters[5].value.ival;
    cidx1 = _data->parameters[6].value.ival;
    xyc = (ae_vector*)_data->parameters[7].value.val;
    xydist2 = (ae_vector*)_data->parameters[8].value.val;
    bufferpool = (ae_shared_pool*)_data->parameters[9].value.val;
    ...
    for(mergeidx=0; mergeidx<=npoints-2; mergeidx++)
    {
        ...
        for(i=0; i<=npoints-1; i++)
        {
            if( i!=c0&&i!=c1 )
            {
                ...
                if( s->ahcalgo==2 )
                {
                    d->ptr.pp_double[i][c0] = ((double)csizes.ptr.p_int[c0]*d->ptr.pp_double[i][c0]+(double)csizes.ptr.p_int[c1]*d->ptr.pp_double[i][c1])/(double)(csizes.ptr.p_int[c0]+csizes.ptr.p_int[c1]);
                }
                if( s->ahcalgo==3 )
                {
                    d->ptr.pp_double[i][c0] = (d->ptr.pp_double[i][c0]+d->ptr.pp_double[i][c1])/(double)2;
                }
                if( s->ahcalgo==4 )
                {
                    d->ptr.pp_double[i][c0] = ((double)(n0+ni)*d->ptr.pp_double[i][c0]+(double)(n1+ni)*d->ptr.pp_double[i][c1]-(double)ni*d01)/(double)(n0+n1+ni);
                }
     ...
     ae_state *_state)
{
    ...
    if( ae_fp_greater_eq(rcomplexity,spawnlevel(_state))&&(i1-i0>2||j1-j0>2) )
    {
        ...
        if( i1-i0>j1-j0 )
        {
            ...
            _spawn_clustering_evaluatedistancematrixrec(xy, nfeatures, disttype, d, i0, i0+len0, j0, j1, _child_tasks, _smp_enabled, _state);
        ...
        else
        {
            ...
            _spawn_clustering_evaluatedistancematrixrec(xy, nfeatures, disttype, d, i0, i1, j0, j0+len0, _child_tasks, _smp_enabled, _state);
    ...
    ae_task_group *_group, ae_bool smp_enabled, ae_state *_state)
{
    if( _group==NULL || !smp_enabled)
    {
        clustering_evaluatedistancematrixrec(xy,nfeatures,disttype,d,i0,i1,j0,j1, _state);
        return;
    }
#if defined(_ALGLIB_HAS_WORKSTEALING)
    {
        ae_task_info *_task;
        _task = ae_create_task(_group, _state);
        _task->data.func = _task_clustering_evaluatedistancematrixrec;
        _task->data.flags = _state->flags;
        _task->data.parameters[0].value.const_val = xy;
        _task->data.parameters[1].value.ival = nfeatures;
        _task->data.parameters[2].value.ival = disttype;
        _task->data.parameters[3].value.val = d;
        _task->data.parameters[4].value.ival = i0;
        _task->data.parameters[5].value.ival = i1;
        _task->data.parameters[6].value.ival = j0;
        _task->data.parameters[7].value.ival = j1;
        ae_push_task(_task, _state, ae_false);
    }
#else
    abort(); /* critical integrity failure */
...
void _task_clustering_evaluatedistancematrixrec(ae_task_data *_data, ae_state *_state)
{
    ...
    ae_int_t j1;
    xy = (const ae_matrix*)_data->parameters[0].value.const_val;
    nfeatures = _data->parameters[1].value.ival;
    disttype = _data->parameters[2].value.ival;
    d = (ae_matrix*)_data->parameters[3].value.val;
    i0 = _data->parameters[4].value.ival;
    i1 = _data->parameters[5].value.ival;
    j0 = _data->parameters[6].value.ival;
    j1 = _data->parameters[7].value.ival;
   ae_state_set_flags(_state, _data->flags);
   clustering_evaluatedistancematrixrec(xy,nfeatures,disttype,d,i0,i1,j0,j1, _state);
}
#endif
